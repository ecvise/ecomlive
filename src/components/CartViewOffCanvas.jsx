...
export default function CartViewOffCanvas({setCart,cart}) {
  ...
  useEffect(() => {
    async function fetchProductData() {
      // You can await here
      const catString = 'carts/user/2'
      const userCart = await fetchData({'dataUrl':catString,'setError':setError,'setLoading':setLoading,'loading':loading,'error':error});
      setCart(userCart[0].products);
      // ...
    }
    fetchProductData();
  }, []);
  ...
  return (
    ...
    >
      ...
      <div className="col-md-3 col-lg-3 col-xl-3">
        <h6 className="text-muted">{item.productID}</h6>
      </div>
...
{/*         <button className="btn btn-link px-2"
      ...
      <div className="col-md-1 col-lg-1 col-xl-1 text-end">
        <a href="#!" className="text-muted" key={item.id} ><i className="fas fa-times"></i></a>
      </div>
     </div>
     <hr className="my-4" />
      </>
